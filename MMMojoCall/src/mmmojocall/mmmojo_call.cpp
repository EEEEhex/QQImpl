...
namespace mmmojocall
{
	...
	bool InitMMMojoGlobal(int argc, const char* const* argv)
	{
		if (g_MMMojoDLL.hMMMojo == NULL) return false;
		g_MMMojoDLL.lpInitializeMMMojo(argc, argv);
		g_is_init_mmmojo = true;
		return true;
	}

	bool ShutdownMMMojoGlobal()
	{
		if (g_MMMojoDLL.hMMMojo == NULL) return false;
		g_MMMojoDLL.lpShutdownMMMojo();
		g_is_init_mmmojo = false;
		return true;
	}
	...
	XPluginManager::XPluginManager()
	{
		...
		m_start_mmmojo_env = false;
	}

	XPluginManager::~XPluginManager()
	{
		if (m_start_mmmojo_env)
		{
			this->StopMMMojoEnv();
		}
	}
